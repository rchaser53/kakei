...
2. 次の行から、店舗名、合計金額、レシート日付をCSVフォーマットで抽出してください。
3. CSVのヘッダーは店舗名を「store_name」、合計金額を「total_amount」、レシート日付を「receipt_date」としてください。
4. 店舗名が明確でない場合は「不明な店舗」と記載してください。
5. 合計金額は数値のみを抽出してください（例: 1200）。
6. レシート日付はYYYY-MM-DD形式で抽出してください（例: 2025-05-16）。日付が不明確な場合は空欄にしてください。
